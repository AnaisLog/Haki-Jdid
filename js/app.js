  ...
  view.innerHTML = `
    <section class="hero">
      <div class="header-flag">
        <div class="flag-stripe k"></div><div class="flag-stripe w"></div><div class="flag-stripe g"></div>
      </div>
      <h1>مرحبا ! MarHaba ! 👋</h1>
...
function lessonCardHtml(lesson, source) {
  ...
  return `
    <button class="lesson-card" data-goto="#/${source}/${lesson.id}">
      <div class="top-row">
        <span class="lesson-icon">${lesson.icon || (source === "arabic" ? "📖" : "🩺")}</span>
...
function renderLesson(view, id, source) {
  ...
  view.innerHTML = `
    ...
    <div class="tip">👆 Clique sur une carte pour voir la traduction · 🔊 clique sur le bouton pour l'entendre en arabe !</div>
    <div class="tts-notice" id="tts-notice-lesson">⚠️ Voix arabe non disponible sur ce navigateur — utilise la transcription phonétique pour t'entraîner.</div>
  ...
  if (typeof ttsReady !== "undefined" && ttsReady && !arabicVoice) {
    const notice = document.getElementById("tts-notice-lesson");
  ...
  view.querySelectorAll(".word-card").forEach(card => {
    card.addEventListener("click", e => {
      if (e.target.closest(".track-btns") || e.target.closest(".speak-btn")) return;
      const wasFlipped = card.classList.contains("flipped");
      card.classList.toggle("flipped");
      if (!wasFlipped) {
        const arabicText = card.querySelector(".arabic")?.textContent;
        speak(arabicText);
      }
    });
  });
  view.querySelectorAll(".speak-btn").forEach(btn => {
    btn.addEventListener("click", e => {
      e.stopPropagation();
      const arabicText = btn.closest(".arabic-row")?.querySelector(".arabic")?.textContent;
      speak(arabicText, btn);
    });
  });
  view.querySelectorAll(".track-btns button").forEach(btn => {
    btn.addEventListener("click", e => {
      ...
      const wasFullyKnown = checkLessonMastered(lesson, id);
      renderLesson(view, id, source); // re-render to update active states
      if (wasFullyKnown) {
        setTimeout(() => showCelebration("📖", "Leçon maîtrisée !", "Tu connais tous les mots de cette fiche ! برافو (bravo) ! 👏"), 200);
      }
    });
  });
}

// Renvoie true si, après la mise à jour en cours, TOUS les mots complets de la leçon sont "known"
function checkLessonMastered(lesson) {
  const progress = getProgress();
  const words = lesson.words.filter(w => w.complete !== false);
  if (!words.length) return false;
  return words.every(w => progress[w.id] === "known");
}
...
function wordCardHtml(w, lessonId) {
  ...
  return `
    <div class="word-card">
      ...
      ${w.emoji ? `<div class="word-emoji-badge">${w.emoji}</div>` : ""}
      <div class="content">
        ...
        <div class="arabic-row">
          ...
          <button class="speak-btn" title="Écouter en arabe">🔊</button>
        </div>
