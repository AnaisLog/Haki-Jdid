...
function renderQuizHome(view, sub) {
  ...
  view.innerHTML = `
    <div class="quiz-intro">
      ...
      <div class="tts-notice" id="tts-notice-quiz">⚠️ Voix arabe non disponible sur ce navigateur — appuie-toi sur la transcription phonétique.</div>
    </div>
  ...
  if (typeof ttsReady !== "undefined" && ttsReady && !arabicVoice) {
    const notice = document.getElementById("tts-notice-quiz");
...
function startQuiz(view, mode, scope) {
  ...
  quizState = { mode, scope, pool, index: 0, score: 0, total: pool.length, flipped: false, streak: [], okCount: 0, noCount: 0 };
  renderQuizStep(view);
}
...
function renderQuizStep(view) {
  ...
  const header = `
    <div class="quiz-runner">
      <div class="score-row">
        <span class="score-pill score-ok">✅ ${quizState.okCount}</span>
        <span class="score-pill score-no">❌ ${quizState.noCount}</span>
      </div>
      <div class="quiz-progress">
        <div class="bar"><div style="width:${pct}%"></div></div>
        <div class="count">${index + 1}/${total}</div>
      </div>
      <div class="streak-bar">${quizState.streak.slice(-10).map(s => `<div class="streak-dot ${s}"></div>`).join("")}</div>
  `;
  ...
  if (mode === "flashcards") {
    view.innerHTML = header + `
      <div class="quiz-card" id="flash-card">
        ...
        <button class="speak-btn" id="flash-speak" title="Écouter en arabe">🔊 Écouter</button>
    ...
    document.getElementById("flash-speak").addEventListener("click", e => {
      e.stopPropagation();
      speak(word.arabic, e.currentTarget);
    });
    card.addEventListener("click", () => {
      ...
      card.innerHTML = `
        ...
        ${word.emoji ? `<div class="reveal-emoji">${word.emoji}</div>` : ""}
    ...
    view.querySelectorAll("[data-result]").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        registerAnswer(btn.dataset.result === "known");
        quizState.index++;
  ...
  if (mode === "qcm-fr" || mode === "qcm-ar") {
    ...
    view.innerHTML = header + `
      <div class="quiz-card">
        ...
        ${isFr
          ? `<div class="arabic-big">${escapeHtml(word.arabic)}</div><div class="phonetic-big">${escapeHtml(word.phonetic)}</div><button class="speak-btn" id="qcm-speak" title="Écouter en arabe">🔊 Écouter</button>`
          : `<div class="prompt-fr">${escapeHtml(word.french)}</div>`
        }
      </div>
    ...
    const qcmSpeakBtn = document.getElementById("qcm-speak");
    if (qcmSpeakBtn) qcmSpeakBtn.addEventListener("click", () => speak(word.arabic, qcmSpeakBtn));
    ...
    view.querySelectorAll(".qcm-options button").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        speak(word.arabic);
        if (word.emoji) {
          const correctBtn = view.querySelector(".qcm-options button.correct");
          if (correctBtn) correctBtn.textContent = `${word.emoji} ${correctBtn.textContent}`;
        }
        setTimeout(() => {
          quizState.index++;
          renderQuizStep(view);
...
function registerAnswer(isCorrect) {
  if (isCorrect) {
    quizState.score++;
    quizState.okCount++;
    quizState.streak.push("ok");
    if (quizState.okCount % 5 === 0) {
      const msgs = [
        ["🎉", "Super série !", "5 bonnes réponses d'affilée !"],
        ["🏆", "Champion !", "Tu maîtrises ton arabe jordanien !"],
        ["🌟", "Impressionnant !", "Continue comme ça !"],
      ];
      const m = msgs[Math.floor(Math.random() * msgs.length)];
      setTimeout(() => showCelebration(m[0], m[1], m[2]), 300);
    }
  } else {
    quizState.noCount++;
    quizState.streak.push("no");
  }
}
...
function renderQuizResult(view) {
  ...
  if (pct >= 90) {
    setTimeout(() => showCelebration("🏆", "Excellent résultat !", `${score}/${total} bonnes réponses — يلا (yalla), continue !`), 250);
  }
}
